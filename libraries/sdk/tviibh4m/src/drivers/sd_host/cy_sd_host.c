...
#else
    #define CY_SD_HOST_CLK_HF_FREQUENCY           (100000000UL) 
#endif
    ...
    if ((NULL != context) && (NULL != config) && (NULL != config->data))
    {
             ...
             ((context->maxSectorNum - dataAddress) < config->numberOfBlocks)))
        {
            ...
            if (CY_SD_HOST_DMA_ADMA2 == context->dmaType)
            {
                ...
                /* Added as bugfix for writeback cache */
                Cy_SysLib_CleanCoreDCacheByAddr((uint32_t*) &aDmaDescriptTbl, sizeof(aDmaDescriptTbl));
...
cy_en_sd_host_status_t Cy_SD_Host_SetSdClkDiv(stc_SDHC_t *base, uint16_t clkDiv)
{
    ...
    if (NULL != base)
    {
        ...
        /* Assure that register is written */
        while (base->CORE.unCLK_CTRL_R.u16Register != temp.u16Register);
